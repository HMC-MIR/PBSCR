...
for num_composers in composer_sets:
    ...
    pretrained_classifier_output_dir = gpt2_dir/f"classifier_{num_composers}_pretrained"
    ...
    config.num_labels = len(label2id)
    config.pad_token_id = tokenizer.pad_token_id
    mode = AutoModelForSequenceClassification.from_config(config)
    tokenizer.model_max_length = config.n_positions
    ...
    nopretrain_classifier_output_dir = gpt2_dir/f"classifier_{num_composers}_nopretrain"
    # Define training arguments
    training_args = TrainingArguments(
